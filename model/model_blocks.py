...
class SRResNet(nn.Module):
    ...
    def __init__(self, large_kernel_size=9, small_kernel_size=3, n_channels=64, n_blocks=16, scaling_factor=4, bands_in=3, bands_out=3):
        """
        ...
        assert scaling_factor in {0, 2, 4, 8}, "The scaling factor must be 2, 4, or 8!"

        # The first convolutional block
        self.conv_block1 = ConvolutionalBlock(in_channels=bands_in, out_channels=n_channels, kernel_size=large_kernel_size,
        ...
        self.conv_block3 = ConvolutionalBlock(in_channels=n_channels, out_channels=bands_out, kernel_size=large_kernel_size,
...
class Generator(nn.Module):
    ...
    def __init__(self, large_kernel_size=9, small_kernel_size=3, n_channels=64, n_blocks=16, scaling_factor=0, bands_in=3,bands_out=3):
        """
        :param large_kernel_size: kernel size of the first and last convolutions which transform the inputs and outputs
        :param small_kernel_size: kernel size of all convolutions in-between, i.e. those in the residual and subpixel convolutional blocks
        :param n_channels: number of channels in-between, i.e. the input and output channels for the residual and subpixel convolutional blocks
        ...
        self.bands_in = bands_in
        self.bands_out = bands_out

        # The generator is simply an SRResNet, as above
        self.net = SRResNet(large_kernel_size=large_kernel_size, small_kernel_size=small_kernel_size,
                            n_channels=n_channels, n_blocks=n_blocks, scaling_factor=scaling_factor, bands_in=self.bands_in, bands_out=self.bands_out)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, kernel_size=3, n_channels=64, n_blocks=8, fc_size=1024, bands_in=3):
        """
...
class TruncatedVGG19(nn.Module):
    ...
    def forward(self, input):
        ...
        if input.shape[1]==1:
            ...
            print("VGG input shape:",input.shape)
